...
- **COBOL parser** (`antcrew_engine.documentation.parsers.cobol.CobolParser`) — parse `.cbl`, `.cob`, `.cpy`, and `.copy` files into the documentation index. Handles fixed-format (cols 1-6 sequence, col 7 indicator) and free-format COBOL. Extracts: `PROGRAM-ID`, `AUTHOR`, `DATE-WRITTEN`, division structure, data items with PIC clauses (WORKING-STORAGE / LINKAGE / FILE sections), paragraph names from PROCEDURE DIVISION, and `COPY` / `CALL` statements.

- **`org_type` field in schema** (`legacy | microservices | mixed`) — controls COBOL-aware behaviour across the documentation module. `is_legacy` property and `CobolSupport` dataclass (`enabled`, `as400_connection`, `copybook_parsing`, `batch_job_docs`). Configured via `if_legacy.cobol_support` in schema YAML.

- **`AS400Connector`** (`antcrew.integrations.as400`) — read-only ODBC connector to IBM AS/400 DB2 for i. Methods: `get_schema(table)` (column metadata via `QSYS2.SYSCOLUMNS`), `get_copybook(table)` (COBOL FD layout derived from schema), `sample_data(table, limit)` (first N rows as list of dicts). Context manager supported. Requires `pip install antcrew[legacy]` (`pyodbc>=4.0.35`).

- **`COBOLAugment`** (`antcrew.augment.cobol`) — add AI to a COBOL program without rewriting it. Takes a `.cbl` file and a plain-English requirement; produces:
  - Python AI wrapper class with `process()` + `_ai_enrich()` / `_ai_interpret()` hooks
  - COBOL bridge caller program that invokes the Python service
  - Markdown deployment guide covering REST adapter, IBM MQ, and batch file exchange patterns
  When an LLM is provided the wrapper is generated by the model; otherwise a typed static template is used.

- **`antcrew augment-cobol`** CLI command — drives `COBOLAugment` from the terminal. Flags: `--requirement` / `-r` (required), `--model` / `-m`, `--output` / `-o`, `--guide`, `--dry-run`.

- **`[legacy]` optional extra** — `pip install antcrew[legacy]` adds `pyodbc>=4.0.35`.
