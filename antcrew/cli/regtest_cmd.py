"""antcrew regtest — prompt mutation regression testing against a TraceLog."""
...
def regtest_cmd(
    ...
    \b
    File-based prompt:
        antcrew regtest ~/.antcrew/trace.db --run <id> --agent BA --prompt v2.txt

    \b
    Inline prompt:
        antcrew regtest ~/.antcrew/trace.db --run <id> --agent BA \\
          --prompt-text "You are a Business Analyst. Be concise."

    \b
    CI gate (fail when diff > 15%):
        antcrew regtest trace.db --run $RUN_ID --agent BA --prompt v2.txt --threshold 0.15
