...
class GeminiModel(BaseLLM):
    ...
    def system_multimodal(
        ...
        *,
        ...
        """Call Gemini with text + images (vision request).

        Args:
            system_prompt: System instruction text.
            user:          User message text.
            images:        List of local file paths or ``http(s)://`` URLs.
                           Local paths are read and base64-encoded.
                           URLs are passed as ``fileUri`` references (Gemini API v1beta).
        ...
        for img in images:
            if img.startswith("http://") or img.startswith("https://"):
                # URL reference — Gemini supports fileUri for publicly accessible URLs
                image_parts.append({"fileData": {"mimeType": "image/jpeg", "fileUri": img}})
            else:
        ...
        user_parts: list[dict] = [{"text": user}] + image_parts
